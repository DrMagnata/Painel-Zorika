...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="css/triagens.css">
            ...
            <input type="text" class="form-control" id="texto" name="texto">
              ...
              <div class="form-group" style="display: none;">
                ...
                <select class="form-control" id="local" name="local">
                  <option value="Sala de triagem Número 02">Triagem 2</option>
                </select>
              </div>
...
try {
    $stmt = $db->prepare("SELECT * FROM chamar WHERE local = 'Sala de triagem Número 02' AND fila = 'Esperando'");
    $stmt->execute();
...
<script>
    async function chamarNovamente(id) {
        try {
            const response = await fetch('triag2.php', {
                method: 'POST',
                headers: {
