            ...
            <input type="text" class="form-control" id="texto" name="texto">
              ...
              <div class="form-group" style="display: none;">
                ...
                <select class="form-control" id="local" name="local">
                  <option value="Sala de triagem Número 01">Triagem 1</option>
                </select>
              </div>
...
try {
    $stmt = $db->prepare("SELECT * FROM chamar WHERE local = 'Sala de triagem Número 01' AND fila = 'Esperando'");
    $stmt->execute();
...
<script>
    async function chamarNovamente(id) {
        try {
            const response = await fetch('triag1.php', {
                method: 'POST',
                headers: {
