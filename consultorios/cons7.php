            ...
            <input type="text" class="form-control" id="texto" name="texto">
              ...
              <div class="form-group" style="display: none;">
                ...
                <select class="form-control" id="local" name="local">
                  <option value="Consultório Número 07">Consultório 7</option>
                </select>
              </div>
...
try {
    $stmt = $db->prepare("SELECT * FROM chamar WHERE local = 'Consultório Número 07' AND fila = 'Esperando'");
    $stmt->execute();
...
<script>
    async function chamarNovamente(id) {
        try {
            const response = await fetch('cons7.php', {
                method: 'POST',
                headers: {
