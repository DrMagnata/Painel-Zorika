            ...
            <input type="text" class="form-control" id="texto" name="texto">
              ...
              <div class="form-group" style="display: none;">
                ...
                <select class="form-control" id="local" name="local">
                  <option value="Consultório Número 08">Consultório 8</option>
                </select>
              </div>
...
try {
    $stmt = $db->prepare("SELECT * FROM chamar WHERE local = 'Consultório Número 08' AND fila = 'Esperando'");
    $stmt->execute();
...
<script>
    async function chamarNovamente(id) {
        try {
            const response = await fetch('cons8.php', {
                method: 'POST',
                headers: {
