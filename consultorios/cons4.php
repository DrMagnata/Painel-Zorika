            ...
            <input type="text" class="form-control" id="texto" name="texto">
              ...
              <div class="form-group" style="display: none;">
                ...
                <select class="form-control" id="local" name="local">
                  <option value="Consultório Número 04">Consultório 4</option>
                </select>
              </div>
...
try {
    $stmt = $db->prepare("SELECT * FROM chamar WHERE local = 'Consultório Número 04' AND fila = 'Esperando'");
    $stmt->execute();
...
<script>
    async function chamarNovamente(id) {
        try {
            const response = await fetch('cons4.php', {
                method: 'POST',
                headers: {
